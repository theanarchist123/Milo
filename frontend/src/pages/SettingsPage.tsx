...
import { User, Shield, CheckCircle2, Server, Key, LogOut } from 'lucide-react';
          ...
          <div className="flex-1 min-w-0 pb-12">
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                <div className="card p-6">
                  ...
                  <div className="flex items-center gap-6 mb-8">
                    <div className="w-20 h-20 rounded-full bg-indigo flex items-center justify-center text-2xl font-bold text-white shadow-xl border-4 border-surface">
            ...
            {activeTab === 'apis' && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                {[
                  { name: 'Google Workspace (Auth)', status: 'Connected', icon: CheckCircle2, color: 'text-success', bg: 'bg-success/10' },
                  { name: 'Gmail API', status: 'Connected', icon: CheckCircle2, color: 'text-success', bg: 'bg-success/10' },
                  { name: 'Google Classroom API', status: 'Connected', icon: CheckCircle2, color: 'text-success', bg: 'bg-success/10' },
                  { name: 'Google Drive API', status: 'Connected', icon: CheckCircle2, color: 'text-success', bg: 'bg-success/10' },
                ].map((api, i) => (
                  <div key={i} className="card p-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center", api.bg)}>
                        <api.icon size={16} className={api.color} />
                      </div>
                      <h4 className="text-sm font-medium text-white">{api.name}</h4>
                    </div>
                    <span className="badge bg-elevated text-text-secondary">{api.status}</span>
                  </div>
                ))}
              </motion.div>
            )}
